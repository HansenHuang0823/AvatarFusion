...
def random_eye_normal():
    ...
    return np.array([
        ...
        camera_distance * np.sin(theta) * np.sin(phi),
        camera_distance * np.cos(theta)
    ]).astype(np.float32), theta, phi, is_front

def random_eye_front(is_front=None, distance=None, theta_std=None):
...
def random_eye(is_front=None, distance=None, theta_std=np.pi / 4, phi_std=np.pi / 8):
    camera_distance = np.random.uniform(1, 2) if distance is None else distance
    theta = np.random.normal(0, theta_std)
    phi = np.random.normal(0, phi_std)
    # is_front = np.random.choice(2) if is_front is None else is_front
    # if is_front == 0:
    #     theta = np.pi - theta
...
def random_eye_body(is_front=None, distance=None, theta_std=None):
    camera_distance = np.random.uniform(1, 2) if distance is None else distance
...
def random_at():
    ...
    ).clip(-0.3, 0.3).astype(np.float32)
...
def at_face():
    return np.array([-0.0055,  0.6461,  0.3525]).astype(np.float32)

def get_view_direction(theta, phi, overhead, front):
    #                   phis [B,];          thetas: [B,]
    # front = 0         [0, front)
    ...
    if y > np.cos(overhead):
        return 'top'
    elif y < -np.cos(overhead):
        return 'bottom'
    else:
        if theta < np.pi/2:
            if theta < front:

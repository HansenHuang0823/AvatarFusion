...
class ClothRenderer:
    ...
    def cat_z_vals(self, rays_o, rays_d, z_vals, new_z_vals, sdf, theta, coe, last=False, is_refine=False):
        ...
        if not last:
            ...
            is_cloth = new_body_sdf >=  (0.00001 if not is_refine else 0.00200)
            is_body = torch.logical_not(is_cloth)
            new_sdf[is_body] = new_body_sdf[is_body]
            new_sdf[is_cloth] = new_cloth_sdf[is_cloth]
    ...
    def render_core(self,
        ...
        # iter_cos_cloth = -(F.relu(-true_cos_cloth * 0.5 + 0.5) * (1.0 - cos_anneal_ratio) +
        #              F.relu(-true_cos_cloth) * cos_anneal_ratio)  # always non-positive
        ...
        alpha = ((p_cloth + 1e-5) / (c_cloth + 1e-5)).reshape(batch_size, n_samples).clip(0.0, 1.0)
        ...
        sampled_color = raw_color_all
        extra_sampled_color = raw_color_all
        ...
        clip_weights_sum = weights_sum.clip(1e-4, 1-1e-4)
        ray_entropy =-(torch.log2(clip_weights_sum)*clip_weights_sum+torch.log2(1-clip_weights_sum)*(1-clip_weights_sum)) / weights_sum.shape[0]
        ...
        return {
            ...
            'sdf_residual': cloth_sdf_residual[is_cloth] - 0.07,
            'color': color,
            'sdf': body_sdf,
            'dists': dists,
            ...
            'weights': weights,
            'cdf': c_cloth.reshape(batch_size, n_samples),
            'gradient_error': gradient_error,
            'inside_sphere': inside_sphere
        }

    def render(self, rays_o, rays_d, near, far, data, coe, perturb_overwrite=-1, background_rgb=None, cos_anneal_ratio=0.0, is_refine=False):
        self.sdf_smpl.eval()
        ...
        if self.n_importance > 0:
            with torch.no_grad():
                ...
                is_cloth = body_sdf >=  (0.00001 if not is_refine else 0.00200)
                is_body = torch.logical_not(is_cloth)

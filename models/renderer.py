...
class NeuSRenderer:
    ...
    def cat_z_vals(self, rays_o, rays_d, z_vals, new_z_vals, sdf, theta, coe, last=False):
        batch_size, n_samples = z_vals.shape
        ...
        if not last:
            new_sdf, pts_cano = self.sdf_smpl(pts.reshape(-1, 3), theta)
            new_sdf = new_sdf.reshape(batch_size, n_importance)
            new_sdf += coe * self.sdf_network.sdf(pts_cano.reshape(-1, 3)).reshape(batch_size, n_importance)
    ...
    def render_core(self,
        ...
        sdf, pts_cano = sdf_smpl(pts, theta)


        sdf_nn_output = sdf_network(pts_cano)
        sdf_residual = sdf_nn_output[:, :1]

        sdf += coe * sdf_residual
        feature_vector = sdf_nn_output[:, 1:]

        gradients_cano = sdf_smpl.cano_gradient(pts_cano)
        gradients_cano += coe * sdf_network.gradient(pts_cano).squeeze()

        jacobian = sdf_smpl.jacobian(pts.clone(), theta).detach()
        gradients = torch.matmul(jacobian, gradients_cano.unsqueeze(-1)).squeeze()


        raw_color = color_network(pts_cano, gradients_cano, dirs, feature_vector).reshape(batch_size, n_samples, 3)
        extra_sampled_color = raw_color
        sampled_color = body_color_network(pts_cano, gradients_cano, dirs, torch.zeros_like(feature_vector)).reshape(batch_size, n_samples, 3)
        ...
        if is_face:
            ...
            head = torch.Tensor([-0.0055,  0.6681,  0.3625]).cuda()
            dis_to_head = torch.linalg.norm(pts - head, ord = 2, dim=-1)
            dis_to_head = dis_to_head.reshape(extra_sampled_color.shape[0:2])
            ratio_of_head = torch.ones_like(dis_to_head)
            ratio_of_head[dis_to_head>0.13] = 1 - (dis_to_head[dis_to_head>0.13]-0.13) / 0.02
            ratio_of_head[dis_to_head>0.15] = 0
            ratio_of_head = ratio_of_head.unsqueeze(-1)
            selected_extra_sampled_color = extra_sampled_color.clone()
            selected_extra_sampled_color = sampled_color * (1-ratio_of_head) + extra_sampled_color * ratio_of_head
            selected_sampled_color = sampled_color.clone()
        color = (selected_extra_sampled_color * weights[:, :, None]).sum(dim=1)

        # if self.extra_color:
        #     extra_color = (selected_extra_sampled_color * weights[:, :, None]).sum(dim=1)
        # else:
        #     extra_color = None
    ...
    def render(self, rays_o, rays_d, near, far, data, coe, perturb_overwrite=-1, background_rgb=None, cos_anneal_ratio=0.0, is_face=False, is_body=False):
        self.sdf_smpl.train()
        ...
        if self.n_importance > 0:
            with torch.no_grad():
                ...
                sdf, pts_cano = self.sdf_smpl(pts.reshape(-1, 3), data["pose"])
                sdf = sdf.reshape(batch_size, self.n_samples)
                sdf += coe * self.sdf_network.sdf(pts_cano.reshape(-1, 3)).reshape(batch_size, self.n_samples)
